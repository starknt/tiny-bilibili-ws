...
onMounted(async () => {
  const room = await fetch(`https://api.live.bilibili.com/room/v1/Room/mobileRoomInit?id=${import.meta.env.VITE_ROOM}`)
    .then(res => res.json())
  const conf = await fetch(`https://api.live.bilibili.com/room/v1/Danmu/getConf?room_id=${room.data.room_id}&platform=pc&player=web`)
    .then(res => res.json())
