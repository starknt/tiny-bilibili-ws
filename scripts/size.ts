...
console.log()
console.log(`browser gzip    ${(gzip(minified) / 1024).toFixed(2)} KiB`)
console.log(`browser brotli  ${(brotli(minified) / 1024).toFixed(2)} KiB`)

minified = ''
const code = await fs.readFile(node, 'utf8')
minified += (await minify(code)).code
console.log()
console.log(`node gzip    ${(gzip(minified) / 1024).toFixed(2)} KiB`)
console.log(`node brotli  ${(brotli(minified) / 1024).toFixed(2)} KiB`)
