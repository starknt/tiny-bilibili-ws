...
export async function getLongRoomId(
  room: number,
  headers?: Record<string, string>,
): Promise<RoomResponse> {
  const res = await request(
    `https://api.live.bilibili.com/room/v1/Room/mobileRoomInit?id=${room}`,
    {
      headers,
    },
  )
...
export async function getDanmuConf(
  query: string,
  headers?: Record<string, string>,
): Promise<DanmuConfResponse> {
  const res = await request(
  ...
  if (data.code !== 0)
    throw new Error(`Failed to get danmu conf: ${data.code} ${data.message}`)
...
export async function getCachedInfo(
  room: number,
  options: BaseLiveClientOptions<any>,
): Promise<[RoomResponse, DanmuConfResponse, string]> {
  ...
  if (cachedRoomInfo.has(room)) {
    ...
    const info = await retry(
      () => getLongRoomId(room, options.headers),
      2,
      200,
    )
  ...
  if (cachedDanmuConf.has(roomInfo.data.room_id)) {
    ...
    const info = await retry(
      () => getDanmuConf(signedQuery, options.headers),
      2,
      200,
    )
